...
const STR = {
  en: {
    tagline: 'Interactive architectural maps of real codebases — click through layers, flows, and dependencies.',
...
function cardHtml(p) {
  ...
  return `<a class="card" href="${href}">
    <div class="card-head"><span class="card-name">${escapeHtml(p.name)}</span>${tag}</div>
...
fetch('./projects.json', { cache: 'no-store' })
  ...
  .then((j) => { all = Array.isArray(j.projects) ? j.projects : []; booted = true; render(); })
